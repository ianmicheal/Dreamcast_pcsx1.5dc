...
int LoadConfig() {
	...
	QueryKeyV(sizeof(Conf->SyncMcds),"SyncMcds",&Conf->SyncMcds);
...
void SaveConfig() {
	...
	SetKeyV("SyncMcds",&Conf->SyncMcds,sizeof(Conf->SyncMcds),REG_DWORD);
...
BOOL CALLBACK ConfigureNetPlayDlgProc(HWND hW, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	switch(uMsg) {
		case WM_INITDIALOG:
	   		Button_SetCheck(GetDlgItem(hW, IDC_SYSCMCDS),  Config.SyncMcds);
		...
		case WM_COMMAND: {
     		switch (LOWORD(wParam)) {
				...
				case IDOK:
					...
					Config.SyncMcds = Button_GetCheck(GetDlgItem(hW,IDC_SYSCMCDS));
