...
/* set a pending branch (target stored in R14) */
#define SetBranch() { \
	branch = 1; \
...
static void recReset() {
	...
	for (i=2; i < 14; i++) regWatch[i].type = REG_FREE;
	regWatch[0].type = REG_RESERVED;
	regWatch[1].type = REG_RESERVED;
	regWatch[14].type = REG_RESERVED;
	regWatch[15].type = REG_RESERVED;
...
define(PROC_OP2R, `void $1_2r(u32 imm, int rs, int rd) {
	u32 r1, r2, r3;	// the sh4 register number

	Lock2Regs(rs, rd);
	
	LoadRegisterR(rs);
	AllocateRegisterR(rd);

	r1 = LoadImmediate32R(imm);
	r2 = mRegs[rs].reg;
	r3 = mRegs[rd].reg;
	...
	// OPCODE(r1, r2)
	$2
	regWatch[r3].changed = 1;
}
')

PROC_OP2R(ADDIU, ADD(r1, r3);)
PROC_OP2R(ANDI, AND(r1, r3);)
PROC_OP2R(ORI, OR(r1, r3);)
PROC_OP2R(XORI, XOR(r1, r3);)
PROC_OP2R(SLTI, CMPGT(r2, r1);MOVT(r3);)
PROC_OP2R(SLTIU, CMPHI(r2, r1);MOVT(r3);)
...
// Rt = Rs + Im
	...
	if (_Imm_) {
		if (_Rs_) {
			ADDIU_2r(_Imm_, _Rs_, _Rt_);
		} else {
			AllocateRegisterR(_Rt_);

			u32 r1 = mRegs[_Rt_].reg;
			LoadImmediate32(_Imm_, r1);
			regWatch[r1].changed = 1;
		}			
	} else {
		if (_Rs_) {
			LoadRegisterR(_Rs_);
			...
			u32 r1 = mRegs[_Rt_].reg;
			u32 r2 = mRegs[_Rs_].reg;
			if (_Rs_ != _Rt_) MOV(r2, r1);
			regWatch[r1].changed = 1;
		} else {
			AllocateRegisterR(_Rt_);

			u32 r1 = mRegs[_Rt_].reg;
			LoadImmediate32(0, r1);
			regWatch[r1].changed = 1;
		}			
	}		
}
...
// Rt = Imm << 16
	...
	u32 r3 = mRegs[_Rt_].reg;
	LoadImmediate32(psxRegs.code << 16, r3);
	regWatch[r3].changed = 1;
}
//End of Load Higher .....
...
/* 5 different cases:
	...
	calculation on the R3000a: rs - rt -> rd	(3 register)
                       sh4:    ra - rb -> ra	(2 register)

	rt   rs   rd
    ------------
    r1   r1   r1		          r1 - r1 -> r1
    r1   r2   r2		          r2 - r1 -> r2
  * r1   r2   r3		r2 -> r3; r3 - r1 -> r3              r3 is rd
    r1   r2   r1		r2 -> r3; r3 - r1 -> r3; r3 -> r1    r3 is a temporary register
															 r3->r1 is not a move, only a mapping

  * r2   r2   r3		r2 -> r3; r3 - r3 -> r3              r3 is rd
*/

define(PROC_OP3R, `void $1_3r(int rt, int rs, int rd) {
	u32 r1, r2, r3;	// the sh4 register number
	...
	if (rs != rd) {
		if (rt == rd) {
			...
			r3 = regGetFree();
			// change the mapping
			regWatch[r1].type = REG_FREE;	// kill r1
			regWatch[r1].changed = 0;
			regWatch[r3].type = REG_REGISTER;	// set r3 to mapping for rd
			regWatch[r3].contents = CalcDispL(GPR.r[rd]);
			regWatch[r3].reg = rd;
			mRegs[rd].reg = r3;
			mRegs[rd].state = ST_MAPPED;
		}
		MOV(r2, r3);
	}
...
PROC_OP3R(ADD, ADD(r1, r3);)
PROC_OP3R(SUB, SUB(r1, r3);)
PROC_OP3R(AND, AND(r1, r3);)
PROC_OP3R(OR, OR(r1, r3);)
PROC_OP3R(XOR, XOR(r1, r3);)
PROC_OP3R(NOR, OR(r1, r3);NOT(r3, r3);)
PROC_OP3R(SLT, CMPGT(r2, r1);MOVT(r3);)
PROC_OP3R(SLTU, CMPHI(r2, r1);MOVT(r3);)
...
// Rd = Rs + Rt 
	...
	if (_Rt_) {
		if (_Rs_) {
			ADD_3r(_Rt_, _Rs_, _Rd_);
		} else {
			u32 r1 = LoadRegisterR(_Rt_);
			u32 r3 = AllocateRegisterR(_Rd_);
			if (_Rt_ != _Rd_) MOV(r1, r3);
			regWatch[r3].changed = 1;
		}			
	} else {
		if (_Rs_) {
			u32 r2 = LoadRegisterR(_Rs_);
			u32 r3 = AllocateRegisterR(_Rd_);
			if (_Rs_ != _Rd_) MOV(r2, r3);
			regWatch[r3].changed = 1;
		} else {
			u32 r3 = AllocateRegisterR(_Rd_);
			LoadImmediate32(0, r3);
			regWatch[r3].changed = 1;
		}			
	}		
}
...
PROC_LOAD(LB,8,EXTSB(R0, r1);)
PROC_LOAD(LBU,8,EXTUB(R0, r1);)
PROC_LOAD(LH,16,EXTSW(R0, r1);)
PROC_LOAD(LHU,16,EXTUW(R0, r1);)
PROC_LOAD(LW,32,MOV(R0, r1);)

define(PROC_STORE, `void rec$1() {
// mem[Rs + Im] = Rt
	...
	if (r2 == 5) {
		if (r1 == 4) { MOV(r2, R1); r2 = 1; }	// worst case
		else { MOV(r2, R4); r2 = 4; }
	}
	...
	CALLFunc((u32)psxMemWrite$2);
}
')

PROC_STORE(SB,8,EXTUB(r1, R5);)
PROC_STORE(SH,16,EXTUW(r1, R5);)
PROC_STORE(SW,32,MOV(r1, R5);)
...
define(PROC_SHIFTI, `void rec$1() {
	if (!_Rd_) return;

	if (_Rt_) {
		Lock2Regs(_Rt_, _Rd_);
		LoadRegisterR(_Rt_);
		AllocateRegisterR(_Rd_);

		u32 r1 = mRegs[_Rt_].reg;
		u32 r3 = mRegs[_Rd_].reg;
...
PROC_SHIFTI(SLL, MOVI(_Sa_, R1);SHLD(R1, r3);)
PROC_SHIFTI(SRL, MOVI(-(_Sa_), R1);SHLD(R1, r3);)
PROC_SHIFTI(SRA, MOVI(-(_Sa_), R1);SHAD(R1, r3);)

//REC_FUNC(SLLV);
//REC_FUNC(SRLV);
//REC_FUNC(SRAV);

define(PROC_SHIFTR, `void rec$1() {
	if (!_Rd_) return;

	if (_Rt_) {
		Lock3Regs(_Rt_, _Rd_, _Rs_);
		LoadRegisterR(_Rt_);
		LoadRegisterR(_Rs_);
		AllocateRegisterR(_Rd_);
		...
		if (_Rs_) {
			MOVI(0x1f, R1);
			AND(r2, R1);
			if (_Rt_ != _Rd_) MOV(r1, r3);		// avoid overwrite of r2 when r3 = r2
			$2
		} else {
			if (_Rt_ != _Rd_) MOV(r1, r3);
		}
...
PROC_SHIFTR(SLLV, SHLD(R1, r3);)
PROC_SHIFTR(SRLV, NEG(R1, R1);SHLD(R1, r3);)
PROC_SHIFTR(SRAV, NEG(R1, R1);SHAD(R1, r3);)
...
// Branch if Rs < 0
	...
	CMPGT(r2, R1);
	j8Ptr[0] = BT(0);
	LoadImmediate32(pc + 4, R14);
	j16Ptr[0] = BRA(0);
	NOP  ();

	shSetBx(j8Ptr[0]);
	LoadImmediate32((_Imm_ * 4 + pc), R14);
...
// Branch if Rs < 0
	...
	shSetBx(j8Ptr[0]);
	LoadImmediate32((_Imm_ * 4 + pc), R14);
	u32 ra = AllocateRegisterR(31);
	LoadImmediate32(pc + 4, ra);
	regWatch[ra].changed = 1;
...
// j target
	...
	LoadImmediate32(_Target_ * 4 + (pc & 0xf0000000), R14);
	SetBranch();
}
...
static void recRecompile() {
	...
	branch = 0;
	regClearJump();  // store all register
