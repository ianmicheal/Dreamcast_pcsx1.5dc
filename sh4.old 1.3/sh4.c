...
/* memwrite and memread addr should be stored in reg */
u32 LoadImmediate32R(u32 imm) {
	/* search if imm somewhere in a reg */
	int r;
	for (r = 2; r < 8; r++) {
		if (regWatch[r].type != REG_IMMEDIATE) continue;
		if (regWatch[r].contents != imm) continue;
//		printf("Found Immediate in R%d %08x\n", r, imm);
		return r;
	}
	r = regGetFree();
//	printf("Put Immediate in R%d %08x\n", r, imm);
	regWatch[r].contents = imm;
	regWatch[r].type = REG_IMMEDIATE;
	regWatch[r].locked = 1;
...
void regClear(u32 ra)
{
	if (regWatch[ra].type == REG_REGISTER) {
		if (regWatch[ra].changed) {
//			printf("Store Register in R%d to %08x\n", ra, regWatch[ra].contents);
			if (regWatch[ra].reg != 0) {
				MOV(ra, R0);
				MOVLSG(regWatch[ra].contents);
			}
		}
//		printf("regClear: %d\n", regWatch[ra].reg);
		mRegs[regWatch[ra].reg].state = ST_UNK;
	}
	regWatch[ra].type = REG_FREE;
	regWatch[ra].changed = 0;
}

/*
void regStore(u32 reg)
{
	if (regWatch[reg].type == REG_REGISTER) {
		if (regWatch[reg].changed) {
			MOV(reg, R0);
			MOVLSG(regWatch[reg].contents);
			regWatch[reg].changed = 0;
		}
	}
}
*/
...
//	int ok = 0;
	...
	for (r = 2; r < 8; r++) {
		if (regWatch[r].type == REG_FREE) { return r; }
	}
//	if (ok) return r;	// found

	do {
		fifo++;
		if (fifo > 7) fifo = 2;
	} while (regWatch[fifo].locked != 0);

//	printf("Kick out a used register %d\n", fifo);
...
{
	int i;
	for (i=2; i < 8; i++) regWatch[i].type = REG_FREE;
	for (i=8; i < 14; i++) regWatch[i].type = REG_NOT_SAVED;

	for (i=0; i < 32; i++) mRegs[i].state = ST_UNK;
}

void regClearJump()
{
	int i;
	for (i=2; i < 8; i++) regClear(i);
}

#define CalcDispL(rx) (((u32)&(psxRegs.rx) - (u32)&psxRegs)>>2)

// don't load register contents from memory because they will be overwritten
// store reg contents
u32 AllocateRegisterR(u32 rx)
{
	u32 disp = CalcDispL(GPR.r[rx]);

	/* search if reg disp somewhere in a sh reg */
	if (mRegs[rx].state == ST_MAPPED) {
		u32 ra = mRegs[rx].reg;
		regWatch[ra].locked = 1;
		return ra;
	}

	int r = regGetFree();
//	printf("Reserve Register in R%d %08x\n", r, disp);
	regWatch[r].contents = disp;
	regWatch[r].reg = rx;
	regWatch[r].type = REG_REGISTER;
	regWatch[r].changed = 0;
	regWatch[r].locked = 1;

	mRegs[rx].reg = r;
	mRegs[rx].state = ST_MAPPED;
	
	return r;
}

// loads a r3000 reg (rx) in a sh4 reg
u32 LoadRegisterR(u32 rx)
{
	u32 disp = CalcDispL(GPR.r[rx]);
...
void Lock3Regs(u32 rx, u32 ry, u32 rz)
{
	if (mRegs[rx].state == ST_MAPPED) { regWatch[mRegs[rx].reg].locked = 1; }
	if (mRegs[ry].state == ST_MAPPED) { regWatch[mRegs[ry].reg].locked = 1; }
	if (mRegs[rz].state == ST_MAPPED) { regWatch[mRegs[rz].reg].locked = 1; }
}

void Lock2Regs(u32 rx, u32 ry)
{
	if (mRegs[rx].state == ST_MAPPED) { regWatch[mRegs[rx].reg].locked = 1; }
	if (mRegs[ry].state == ST_MAPPED) { regWatch[mRegs[ry].reg].locked = 1; }
}
