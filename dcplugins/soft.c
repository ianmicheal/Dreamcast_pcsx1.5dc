...
// fast solid loops... yeah, FAST!... a bit more additional code, of course
...
#define GET_PDEST pdest = xdest[0] | (xdest[1]<<16);
#define SET_PDEST xdest[0] = pdest & 0xffff; xdest[1] = pdest>>16;
...
__inline void GetShadeTransCol32(unsigned short * xdest,unsigned long color)
{
 unsigned long pdest;
 GET_PDEST;
...
__inline void GetTextureTransColG32(unsigned short * xdest,unsigned long color)
{
 long r,g,b,l;
 unsigned long pdest;
 GET_PDEST;
 ...
 if((color&0xffff)==0    ) {pdest=(pdest&0xffff)|(((X32PSXCOL(r,g,b))|l)&0xffff0000); SET_PDEST; return;}
 if((color&0xffff0000)==0) {pdest=(pdest&0xffff0000)|(((X32PSXCOL(r,g,b))|l)&0xffff); SET_PDEST; return;}
...
__inline void GetTextureTransColG32_S(unsigned short * pdest,unsigned long color)
{
 long r,g,b;
 ...
 if((color&0xffff)==0)     {pdest[1]=(((X32PSXCOL(r,g,b))|lSetMask)>>16);return;}
 if((color&0xffff0000)==0) {pdest[0]=(((X32PSXCOL(r,g,b))|lSetMask)&0xffff);return;}

 pdest[0] = ((X32PSXCOL(r,g,b))|lSetMask)&0xffff;
 pdest[1] = ((X32PSXCOL(r,g,b))|lSetMask)>>16;
 
// *pdest=(X32PSXCOL(r,g,b))|lSetMask;
}

////////////////////////////////////////////////////////////////////////

__inline void GetTextureTransColG32_SPR(unsigned short * xdest,unsigned long color)
{
 long r,g,b;
 unsigned long pdest;
 GET_PDEST;
 ...
 if((color&0xffff)==0    ) {pdest=(pdest&0xffff)|(((X32PSXCOL(r,g,b))|lSetMask)&0xffff0000);SET_PDEST;return;}
 if((color&0xffff0000)==0) {pdest=(pdest&0xffff0000)|(((X32PSXCOL(r,g,b))|lSetMask)&0xffff);SET_PDEST;return;}
...
__inline void GetTextureTransColGX32(unsigned short * xdest,unsigned long color,short m1,short m2,short m3)
{
 long r,g,b,l;
 unsigned long pdest;
 GET_PDEST;
 ...
 if(DrawSemiTrans && (color&0x80008000))
  {
   ...
   if(GlobalTextABR==0)
    {                                                 
     r=((((X32TCOL1(pdest))+((X32COL1(color)) * m1))&0xFF00FF00)>>8);
     b=((((X32TCOL2(pdest))+((X32COL2(color)) * m2))&0xFF00FF00)>>8);
     g=((((X32TCOL3(pdest))+((X32COL3(color)) * m3))&0xFF00FF00)>>8);
    }
   else
   if(GlobalTextABR==1)
    {
     r=(X32COL1(pdest))+(((((X32COL1(color)))* m1)&0xFF80FF80)>>7);
     b=(X32COL2(pdest))+(((((X32COL2(color)))* m2)&0xFF80FF80)>>7);
     g=(X32COL3(pdest))+(((((X32COL3(color)))* m3)&0xFF80FF80)>>7);
    }
   else
   if(GlobalTextABR==2)
    {
    ...
    {
     r=(X32COL1(pdest))+(((((X32ACOL1(color))>>1)* m1)&0xFF80FF80)>>7);
     b=(X32COL2(pdest))+(((((X32ACOL2(color))>>1)* m2)&0xFF80FF80)>>7);
     g=(X32COL3(pdest))+(((((X32ACOL3(color))>>1)* m3)&0xFF80FF80)>>7);
    }

   if(!(color&0x8000))
    {
     r=(r&0xffff0000)|((((X32COL1(color))* m1)&0x0000FF80)>>7);
     b=(b&0xffff0000)|((((X32COL2(color))* m2)&0x0000FF80)>>7);
     g=(g&0xffff0000)|((((X32COL3(color))* m3)&0x0000FF80)>>7);
    }
   if(!(color&0x80000000))
    {
     r=(r&0xffff)|((((X32COL1(color))* m1)&0xFF800000)>>7);
     b=(b&0xffff)|((((X32COL2(color))* m2)&0xFF800000)>>7);
     g=(g&0xffff)|((((X32COL3(color))* m3)&0xFF800000)>>7);
    }
  }
 else 
  {
...
__inline void GetTextureTransColGX32_S(unsigned short * pdest,unsigned long color,short m1,short m2,short m3)
{
 long r,g,b;
                      ...
                      short y1,unsigned short col)
{
 ...
 if(y0>MAXYLINESMIN1) return;
 if(x0>1023)          return;

 if(y1>MAXYLINES)  y1=MAXYLINES;
 if(x1>1024)       x1=1024;
   ...
   if(!bCheckMask && !DrawSemiTrans)
    {
     for(i=0;i<dy;i++)
      {
       for(j=0;j<dx;j++) {
       	 *((unsigned short*)DSTPtr)=lcol & 0xffff;
       	 *(((unsigned short*)DSTPtr)+1)=lcol>>16;
       	 DSTPtr++;
       }
       DSTPtr += LineOffset;
      }
...
__inline BOOL SetupSections_G(short x1,short y1,short x2,short y2,short x3,short y3,long rgb1, long rgb2, long rgb3)
{
 ...
 v1 = vtx;   v1->x=x1<<16;v1->y=y1;
 v1->R=(rgb1>>3) & 0x001f0000;
 v1->G=(rgb1<<5) & 0x001f0000;
 v1->B=(rgb1<<13) & 0x001f0000;
 v2 = vtx+1; v2->x=x2<<16;v2->y=y2;
 v2->R=(rgb2>>3) & 0x001f0000;
 v2->G=(rgb2<<5) & 0x001f0000;
 v2->B=(rgb2<<13) & 0x001f0000;
 v3 = vtx+2; v3->x=x3<<16;v3->y=y3;
 v3->R=(rgb3>>3) & 0x001f0000;
 v3->G=(rgb3<<5) & 0x001f0000;
 v3->B=(rgb3<<13) & 0x001f0000;
...
//       *((unsigned long *)&psxVuw[(i<<10)+j])=lcolor;
		psxVuw[(i<<10)+j]=lcolor&0xffff;
		psxVuw[(i<<10)+j+1]=lcolor>>16;
      }
...
//         *((unsigned long *)&psxVuw[(i<<10)+j])=
			unsigned long rtmp =
            ((((cR1+difR) <<10)&0x7c000000)|(((cG1+difG) << 5)&0x03e00000)|(((cB1+difB))&0x001f0000)|
             (((cR1) >> 6)&0x7c00)|(((cG1) >> 11)&0x03e0)|(((cB1) >> 16)&0x001f))|lSetMask;
            psxVuw[(i<<10)+j]=rtmp&0xffff;
            psxVuw[(i<<10)+j+1]=rtmp>>16;
         cR1+=difR2;
         cG1+=difG2;
         cB1+=difB2;
        }
       if(j==xmax)
        psxVuw[(i<<10)+j]=(((cR1 >> 6)&0x7c00)|((cG1 >> 11)&0x03e0)|((cB1 >> 16)&0x001f))|sSetMask;
      }
     if(NextRow_G()) return;
    }
   return;
   ...
   if(xmax>=xmin)
    {
     ...
     for(j=xmin;j<xmax;j+=2) 
      {
       GetShadeTransCol32((unsigned long *)&psxVuw[(i<<10)+j],
        (((cR1+difR) <<10)&0x7c000000)|(((cG1+difG) << 5)&0x03e00000)|(((cB1+difB))&0x001f0000)|
        (((cR1) >> 6)&0x7c00)|(((cG1) >> 11)&0x03e0)|(((cB1) >> 16)&0x001f));
     ...
     if(j==xmax)
      GetShadeTransCol(&psxVuw[(i<<10)+j],((cR1 >> 6)&0x7c00)|((cG1 >> 11)&0x03e0)|((cB1 >> 16)&0x001f));
    }
   if(NextRow_G()) return;
  }
}
     ...
     for(j=xmin;j<xmax;j+=2) 
      {
       ...
       tC2 = psxVub[(((posY+difY)>>5)&0xFFFFF800)+YAdjust+
                    (XAdjust>>1)];
       tC2=(tC2>>((XAdjust&1)<<2))&0xf;
       GetTextureTransColGX32((unsigned long *)&psxVuw[(i<<10)+j],
            psxVuw[clutP+tC1]|
            ((long)psxVuw[clutP+tC2])<<16,
            (cB1>>16)|((cB1+difB)&0xff0000),
            (cG1>>16)|((cG1+difG)&0xff0000),
     ...
     switch (GlobalTextTP)
      {
       case 0:
        drawPoly4TEx4_TRI(lx0,ly0,lx1,ly1,lx3,ly3,lx2,ly2,
                      (gpuData[2] & 0x000000ff), ((gpuData[2]>>8) & 0x000000ff), (gpuData[4] & 0x000000ff), ((gpuData[4]>>8) & 0x000000ff),(gpuData[8] & 0x000000ff), ((gpuData[8]>>8) & 0x000000ff),(gpuData[6] & 0x000000ff), ((gpuData[6]>>8) & 0x000000ff), ((gpuData[2]>>12) & 0x3f0), ((gpuData[2]>>22) & 0x1ff));
        return;
       case 1:
 ...
 switch (GlobalTextTP)
  {
   case 0: // texture is 4-bit
    ...
    for (sprCY=0;sprCY<sprtH;sprCY++)
     for (sprCX=0;sprCX<sprtW;sprCX++)
      {
       tC= psxVub[((textY0+sprCY)<<11) + textX0 -sprCX];
   ...
   case 1: 
    ...
    for(sprCY=0;sprCY<sprtH;sprCY++)
     for(sprCX=0;sprCX<sprtW;sprCX++)
      { 
       tC = psxVub[((textY0+sprCY)<<11)+(GlobalTextAddrX<<1) + textX0 - sprCX] & 0xff;
     ...
     for (sprCX=0;sprCX<sprtW;sprCX++)
      { 
       GetTextureTransColG_SPR(&psxVuw[((sprtY+sprCY)<<10)+sprtX+sprCX],
           psxVuw[((textY0+sprCY)<<10)+GlobalTextAddrX + textX0 - sprCX]);
      }
     return;
  }
}
