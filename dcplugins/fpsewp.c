...
char buf[12];
    ...
    sprintf(buf,"%i",iUseNoStrechBlt);
    g->WriteCfg("GpuPeopsSDL","iUseNoStrechBlt", buf);
}
...
int ReadConfig(void){
    ...
    if (g->ReadCfg("GpuPeopsSDL","iUseNoStrechBlt", myvalue) == FPSE_OK && strlen(myvalue)) {
		iUseNoStrechBlt=atoi(myvalue);}
	    else return FPSE_ERR;
    return FPSE_OK;
}
#endif
